...
PPU.params["gridA"] = lvec_t[1]
PPU.params["gridB"] = lvec_t[2]
PPU.params["gridC"] = lvec_t[3]  # must be before parseAtoms
print(
    PPU.params["gridN"], PPU.params["gridA"], PPU.params["gridB"], PPU.params["gridC"]
)
...
for path in paths:
    ...
    Fx, Fy, Fz, E = fFFT.potential2forces_mem(
        rho1, lvec1, rho1.shape, rho=rho_tip, doForce=True, doPot=False, deleteV=True
    )
