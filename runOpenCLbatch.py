...
def updateFF_LJC(
    ff_args, iZPP, xyzs, Zs, qs, typeParams, pbcnx=0, func_runFF=FFcl.runLJC
):
...
if __name__ == "__main__":
    ...
    for geomFileName in geomFileNames:
        ...
        izslice = int(
            round((rmax[2] - xdirmax - rSliceAbove[2]) / -oclr.DEFAULT_dTip[2])
        )
        print(
            "izslice ",
            izslice,
        ...
        cl.enqueue_copy(
            oclr.oclu.queue, relax_args[0], FEin, origin=(0, 0, 0), region=region
        )
        FEout = oclr.relax(
            relax_args,
            relax_dim,
