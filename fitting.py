...
iZs, Rs, Qs = PPH.parseAtoms(
    atoms, autogeom=False, PBC=PPU.params["PBC"], FFparams=FFparams
)
...
def comp_msd(x=[]):
    ...
    fzs, PPpos, PPdisp, lvecScan = PPH.perform_relaxation(
        lvec, FFLJ, FFel, FFboltz, tipspline=None
    )
    ...
    with open("iteration.txt", "a") as myfile:
        myfile.write(
            "iteration {}: {} rmsd: {} max dev: {} min dev: "
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        action="store",
        type="float",
        help="Fit " "the gaussian width of the charge distribution",
        default=None,
    )
    parser.add_option(
        "-k",
        ...
        action="store",
        type="float",
        help="Fit " "the lateral stiffness of the PP",
        default=None,
    )
    parser.add_option(
        "--krad",
        action="store",
        type="float",
        help="Fit " "the radial stiffness of the PP",
        default=None,
    )
    parser.add_option(
        "-a",
        ...
        action="append",
        type="string",
        help="Fit " "the LJ parameters of the given atom",
        default=None,
        nargs=3,
    )
    parser.add_option(
        "--nobounds",
        action="store_true",
        help="Skipf the first optimization step with bounds",
        default=False,
    )
