...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        vb = QtWidgets.QHBoxLayout()
        l0.addLayout(vb)
        bt = QtWidgets.QPushButton("Load .npy", self)
        bt.setToolTip("load .npy file  from dir")
        bt.clicked.connect(self.loadNPY)
        vb.addWidget(bt)
        self.btLoad = bt
    ...
    def magicFit(self):
        ...
        if iz < len(self.data) - 1:
            ...
            self.data[iz] = nimg.shift(
                self.data[iz],
                (-ix - self.shifts[iz][0], -iy - self.shifts[iz][1]),
                order=3,
                mode="mirror",
            )
            self.shifts[iz][0] = -ix
    ...
    def loadData(self):
        ...
        for z_slice in range(len(self.data)):
            if slice_lengths[z_slice][0] != max_length:
                print(
                    "slice_lengths/max_lengths = ",
                    slice_lengths[z_slice][0] / max_length,
                )
                scaled_size = int(
                    image_shape[0] * slice_lengths[z_slice][0] / max_length
                )
                scaled_image_slice = np.zeros_like(data[z_slice])
                start_xy = int((image_shape[0] - scaled_size) / 2)
                scaled_image_slice[
                    start_xy : start_xy + scaled_size, start_xy : start_xy + scaled_size
                ] = np.array(
                    scipy.misc.imresize(self.data[z_slice], (scaled_size, scaled_size))
                )
                self.data[z_slice] = scaled_image_slice
                self.margins = [
                    start_xy,
                    image_shape[0] - scaled_size - start_xy,
                    image_shape[1] - scaled_size - start_xy,
                    start_xy,
                ]
        ...
        for z_slice in range(len(self.data)):
            marged_size = (
                image_shape[0] - imarginx0 - imarginx1,
                image_shape[1] - imarginy0 - imarginy1,
            )
            # print 'marged_size =',marged_size
            # print 'self.max_length  =', max_length
            slice_lengths[z_slice] = [
                marged_size[0] * max_length / image_shape[0],
                marged_size[1] * max_length / image_shape[1],
            ]
    ...
    def marginData(self):
        ...
        for z_slice in range(len(self.data)):
            ...
            self.slice_lengths[z_slice] = [
                marged_size[0] * self.max_length / image_shape[0],
                marged_size[1] * self.max_length / image_shape[1],
            ]
