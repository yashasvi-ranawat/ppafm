...
parser = OptionParser()
parser.add_option(
    "-i", action="store", type="string", help="input file name", default="df"
)
parser.add_option(
    "--cmap", action="store", type="string", help="input file name", default="gray"
)
parser.add_option(
    "--izs",
    action="store",
    type="int",
    help="Range of plotted z slice indexes",
    nargs=3,
)
parser.add_option(
    "--vrange", action="store", type="float", help="Range of values", nargs=2
)
parser.add_option(
    "--npy",
    action="store_true",
...
parser.add_option(
    ...
    action="store",
    type="string",
    help="xyz geometry file",
    default="input_plot.xyz",
)
parser.add_option(
...
for ii, i in enumerate(slices):
    ...
    if options.vrange:
        plt.imshow(
            ...
            cmap=options.cmap,
            extent=extent,
            vmin=options.vrange[0],
            vmax=options.vrange[1],
        )
    else:
        plt.imshow(
            data[i],
