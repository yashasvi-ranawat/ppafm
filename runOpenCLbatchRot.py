...
if __name__ == "__main__":
    ...
    for dirName in dirNames:
        ...
        for irot, rot in enumerate(rotations):
            ...
            print("pos0", pos0, "\nrot", rot, "\ndTip", dTip, "\ndpos0", dpos0)
            relax_poss = oclr.preparePossRot(
                relax_dim, pos0, rot[0], rot[1], start=(-10.0, -10.0), end=(10.0, 10.0)
            )
            print("relax_poss.shape ", relax_poss.shape)
            writeDebugXYZ(
                subDirName + "/debugPos0.xyz",
                atom_lines,
                relax_poss[::10, ::10, :].reshape(-1, 4),
            )
            # continue
            # exit()
            ...
            print(
                "Timing[s] Ttot %f Tff %f Trelax %f Tprepare %f Tplot %f "
                % (Ttot, Tff, Trelax, Tprepare, Tplot)
            )
