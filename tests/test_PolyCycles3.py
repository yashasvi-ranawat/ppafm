...
def generateAromaticMolecule(
    ...
    epairProb=0.4,
    bNitrogenFix=True,
    fname="test_PolyCyclesMMFF.xyz",
    bMovie=False,
):
    T1()
    ring_pos, ring_Rs, Lrange = generateRings(
        N=30, ngons=[5, 6, 7], ngon_rates=[0.2, 0.6, 0.2], sz=2.5
    )
    T2("generateRings")
    T1()
    atom_pos, bonds, atypes, nngs, neighs, ring_bonds, atom_N6mask = ch.ringsToMolecule(
        ring_pos, ring_Rs
    )
    T2("ch.ringsToMolecule")
    T1()
    bondOrder, atomOrder, _ = ch.estimateBondOrder(
        atypes, bonds, E12=0.5, E22=+0.5, E32=+0.5
    )
    T2("ch.estimateBondOrder")

    T1()
    apos = (
        np.append(atom_pos, np.zeros((len(atom_pos), 1)), axis=1) * bondLenght
    ).copy()
    npis, nepairs, elems = mmff.assignAtomTypes(
        atomOrder, nngs, epairProb=epairProb, bNitrogenFix=True, ne2elem=["C", "N", "O"]
    )
    apos, atypes, aelems = mmff.relaxMolecule(
        apos, bonds, npis, nepairs, elems, fname=fname, bMovie=bMovie
    )
