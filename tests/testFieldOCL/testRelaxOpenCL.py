...
def loadFEcl(Q=None):
    ...
    FE = np.zeros(E.shape + (4,), dtype=np.float32)
    FE.shape
    FE[:, :, :, 3] = E
    Fx, lvec, nDim, head = GU.loadXSF("FLJx_cl.xsf")
    FE[:, :, :, 0] = Fx
    Fy, lvec, nDim, head = GU.loadXSF("FLJy_cl.xsf")
    FE[:, :, :, 1] = Fy
    Fz, lvec, nDim, head = GU.loadXSF("FLJz_cl.xsf")
    FE[:, :, :, 2] = Fz
    if Q is not None:
        E, lvec, nDim, head = GU.loadXSF("Eel_cl.xsf")
        FE[:, :, :, 3] += Q * E
        Fx, lvec, nDim, head = GU.loadXSF("Felx_cl.xsf")
        FE[:, :, :, 0] += Q * Fx
        Fy, lvec, nDim, head = GU.loadXSF("Fely_cl.xsf")
        FE[:, :, :, 1] += Q * Fy
        Fz, lvec, nDim, head = GU.loadXSF("Felz_cl.xsf")
        FE[:, :, :, 2] += Q * Fz
    cell = lvec[1:4, 0:3]
    invCell = np.linalg.inv(cell)
    print(invCell)
    invA = np.zeros(4, dtype=np.float32)
    invA[0:3] = invCell[0]
    invB = np.zeros(4, dtype=np.float32)
    invB[0:3] = invCell[1]
    invC = np.zeros(4, dtype=np.float32)
    invC[0:3] = invCell[2]
...
CL_PATH = os.path.normpath(THIS_FILE_PATH + "/../../cl")
print(CL_PATH)
...
t2 = time.clock()
print("relaxStrokes time %f [s]" % (t2 - t1))
...
lvec_OUT = ([0.0, 0.0, 0.0], [10.0, 0.0, 0.0], [0.0, 10.0, 0.0], [0.0, 0.0, 6.0])
