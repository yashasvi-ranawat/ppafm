...
taffins = np.array(
    [-4.5280, -4.5280, -3.006, -3.006, -5.343, -5.343, -6.899, -8.741, -10.874]
)
thards = np.array(
    [13.8904, 13.8904, 4.772, 4.772, 10.126, 10.126, 11.760, 13.364, 14.948]
)
...
if __name__ == "__main__":
    ...
    for i in range(N1, N2):
        ...
        itypes = (np.random.rand(natom) * 1.3).astype(np.int32)
        ...
        plt.scatter(
            xyzs[:, 0], xyzs[:, 1], c=qs, alpha=1.0, vmin=-0.5, vmax=0.5, cmap="bwr"
        )
        plt.colorbar()
        plt.show()
