...
parser = OptionParser()
parser.add_option(
    "--noProbab", action="store_false", help="probability False", default=True
)
...
def getXYZ(nDim, cell):
    ...
    print(" dcell ", dcell)
    CBA = np.mgrid[0 : nDim[0], 0 : nDim[1], 0 : nDim[2]].astype(
        float
    )  # grid going: CBA[z,x,y]
...
if options.noProbab:
    ...
    # --- sample
    V_surf, lvec, nDim = GU.load_scal_field(
        "sample/VLJ", data_format=options.data_format
    )
...
if options.noForces:
    ...
    if options.noProbab == False:
        ...
        # --- sample
        W_surf, lvec, nDim = GU.load_scal_field(
            "W_surf", data_format=options.data_format
        )

    W_tip = np.roll(
        np.roll(np.roll(W_tip, nDim[0] / 2, axis=0), nDim[1] / 2, axis=1),
        nDim[2] / 2,
        axis=2,
    )  # works for tip (the last flexible tip apex atom) in the middle of the cell
    ...
    # Fz:
    Fz_tip = np.roll(
        np.roll(np.roll(Fz_tip, nDim[0] / 2, axis=0), nDim[1] / 2, axis=1),
        nDim[2] / 2,
        axis=2,
    )  # works for tip (the last flexible tip apex atom) in the middle of the cell
    ...
    # Fx:
    Fx_tip = np.roll(
        np.roll(np.roll(Fx_tip, nDim[0] / 2, axis=0), nDim[1] / 2, axis=1),
        nDim[2] / 2,
        axis=2,
    )  # works for tip (the last flexible tip apex atom) in the middle of the cell
    ...
    # Fy:
    Fy_tip = np.roll(
        np.roll(np.roll(Fy_tip, nDim[0] / 2, axis=0), nDim[1] / 2, axis=1),
        nDim[2] / 2,
        axis=2,
    )  # works for tip (the last flexible tip apex atom) in the middle of the cell
...
if options.current:
    ...
    print("nDim ", nDim, "\ncell ", cell)
    X, Y, Z = getXYZ(nDim, cell)
    T_tip = getProbeTunelling(
        (cell[0, 0] / 2.0 + cell[1, 0] / 2.0, cell[1, 1] / 2, cell[2, 2] / 2.0),
        X,
        Y,
        Z,
        beta=1.14557,
    )  # beta decay in eV/Angstom for WF = 5.0 eV;  works for tip (the last flexible tip apex atom) in the middle of the cell
    T_tip = np.roll(
        np.roll(np.roll(T_tip, nDim[0] / 2, axis=0), nDim[1] / 2, axis=1),
        nDim[2] / 2,
        axis=2,
    )
    ...
    print((T**2).shape)
    GU.save_scal_field(
        "I_boltzmann", T**2, lvec, data_format=options.data_format
    )  # I ~ T**2
