...
parser = OptionParser()
parser.add_option(
    "-i", action="store", type="string", help="input file", default="CHGCAR"
)
parser.add_option(
    "-o",
    action="store",
...
parser.add_option(
    ...
    action="store",
    type="float",
    help="cut-out atoms bellow this height",
    default=-100.0,
)
parser.add_option(
    "--height",
    action="store",
    type="float",
    help="how far above atom is isosurface fitted",
    default=+5.0,
)
parser.add_option(
    "--old", action="store_true", help="use old version of params.ini", default=False
)
parser.add_option("--plot", action="store_false", help="not-plot ?", default=True)
parser.add_option(
    "--debug",
    action="store_true",
    help="plot and pr. all lines and data from fit",
    default=False,
)
...
def getIsoArg(zs_, fs, iso=0.01, atom_z=0.0):
    ...
    if (
        options.debug
        or x0 + dx * (iso - f0) / df >= 5.0
        or x0 + dx * (iso - f0) / df <= 1.0
    ):
        print("atom_z", atom_z)
        print("zs_[0]", zs_[0])
...
for i in ilist:
    ...
    if not (0.5 < Riso < 5.0):
        print(
            "!!! Problem with Riso for atom no. %i : Riso %f, we will use tabled number."
            % (i, Riso)
        )
        Riso = REAs[i][0]
    f1.write(
