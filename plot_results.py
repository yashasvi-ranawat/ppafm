...
parser.add_option(
    "--df", action="store_true", default=False, help="plot images for dfz "
)
parser.add_option(
    "--save_df",
    action="store_true",
    default=False,
    help="save frequency shift as df.xsf ",
)
parser.add_option(
    "--Laplace",
    action="store_true",
...
parser.add_option(
    "--pos", action="store_true", default=False, help="save probe particle positions"
)
parser.add_option(
    "--atoms", action="store_true", default=False, help="plot atoms to images"
)
parser.add_option(
    "--bonds", action="store_true", default=False, help="plot bonds to images"
)
parser.add_option(
    "--cbar", action="store_true", default=False, help="plot bonds to images"
)
parser.add_option(
    "--WSxM",
    action="store_true",
    default=False,
    help="save frequency shift into WsXM *.dat files",
)
parser.add_option(
    "--bI", action="store_true", default=False, help="plot images for Boltzmann current"
)
parser.add_option(
    "-f",
    "--data_format",
...
parser.add_option(
    "--noPBC", action="store_false", help="pbc False", dest="PBC", default=None
)
...
for iq, Q in enumerate(Qs):
    for ik, K in enumerate(Ks):
        ...
        if opt_dict["iets"] is not None:
            try:
                eigvalK, lvec, nDim = GU.load_vec_field(
                    dirname + "/eigvalKs", data_format=options.data_format
                )
                M = opt_dict["iets"][0]
                E0 = opt_dict["iets"][1]
                w = opt_dict["iets"][2]
        ...
        if opt_dict["df"] or opt_dict["save_df"] or opt_dict["WSxM"]:
            try:
                fzs, lvec, nDim = GU.load_scal_field(
        ...
        if opt_dict["bI"]:
            try:
                ...
                print(
                    "cannot load : "
                    + (dirname + "/OutI_boltzmann." + options.data_format)
                )
