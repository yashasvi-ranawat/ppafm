...
atom_types, atom_pos = GU.getFromHead_PRIMCOORD(
    head
)  # load atoms from header of xsf file
...
# print FFparams
atom_Rmin, atom_Rmax = MP.make_Ratoms(
    atom_types, np.array([p[0] for p in FFparams]), fmax=3.0
)
...
sampled_val, sampled_pos = MP.sampleGridArroundAtoms(
    atom_pos, atom_Rmin, atom_Rmax, atom_mask, pbc=False, show_where=True
)
# GU.saveXSF( 'LOCPOT_sample.xsf', V, lvec, head );

centers = atom_pos[atom_mask].copy()
print("centers", centers)
types = np.array([3] * len(centers)).astype(np.uint32)
print("types", types)

B, BB = MP.buildLinearSystemMultipole(sampled_pos, sampled_val, centers, types)
print("B=", B)
print("BB=", BB)

coefs = np.linalg.solve(BB, B)
print("coefs:", coefs)
