...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def updateFF(self):
        ...
        Zs, xyzs, qs = PPU.PBCAtoms(
            Zs, xyzs, qs, avec=self.lvec[1], bvec=self.lvec[2], na=0, nb=0
        )
        ...
        if self.mode == Modes.LJQ.name:
            ...
            self.ff_args = FFcl.updateArgsLJC(
                self.ff_args, self.atoms, self.cLJs, self.poss
            )
    ...
    def loadInputs_New(self):
        ...
        if self.mode == Modes.LJQ.name:
            ...
            lines = [
                "%i %f %f %f %f"
                % (atoms[0][i], atoms[1][i], atoms[2][i], atoms[3][i], atoms[3][i])
    ...
    def getFF(self):
        ...
        self.plot_FF = True
        t2 = time.clock()
        print("FFcl.runLJC time %f [s]" % (t2 - t1))
        # self.plotSlice()
        self.updateDataView()
    ...
    def F2df(self):
        ...
        if nAmp > 0:
            t1 = time.clock()
            self.df = -PPU.Fz2df(
                np.transpose(self.FEout[:, :, :, 2], (2, 0, 1)),
