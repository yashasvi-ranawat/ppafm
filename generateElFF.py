...
if __name__ == "__main__":
    ...
    parser = OptionParser()
    parser.add_option(
        "-i", "--input", action="store", type="string", help="format of input file"
    )
    parser.add_option(
        "--tip_dens",
        action="store",
        type="string",
        default=None,
        help="tip enisty file (.xsf)",
    )
    # parser.add_option( "--sub_core",  action="store_true",  help="subtract core density", default=False )
    parser.add_option(
        "--Rcore",
        default=PPU.params["Rcore"],
        action="store",
        type="float",
        help="Width of nuclear charge density blob to achieve charge neutrality [Angstroem]",
    )
    parser.add_option(
        "-t",
        "--tip",
    ...
    parser.add_option(
        "-E", "--energy", action="store_true", help="pbc False", default=False
    )
    parser.add_option(
    ...
    parser.add_option(
        ...
        action="store",
        type="float",
        help="gaussian width for convolution in Electrostatics [Angstroem]",
        default=None,
    )
    parser.add_option(
        "-f",
    ...
    print(
        ">>> calculating electrostatic forcefiled with FFT convolution as Eel(R) = Integral( rho_tip(r-R) V_sample(r) ) ... "
    )
    ...
    GU.save_vec_field(
        "FFel", FFel, lvec_samp, data_format=options.data_format, head=head_samp
    )
    if options.energy:
