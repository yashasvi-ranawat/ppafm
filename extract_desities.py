...
parser = OptionParser()
parser.add_option(
    "-i", action="store", type="string", help="input file", default="CHGCAR.xsf"
)
parser.add_option(
    "-n",
    "--same_name",
    action="store_false",
    help="save the file with same name as input file",
    default=True,
)
parser.add_option(
    "--zmin",
    action="store",
    type="float",
    help="From what height to extract densities",
    default=+0.0,
)
parser.add_option(
    "--zmax",
    action="store",
    type="float",
    help="To which height extract densities",
    default=-1.0,
)
parser.add_option(
    "--dz", action="store", type="float", help="use dz with step", default=-0.1
)
...
zs = np.arange(
    options.zmin,
    options.zmax if options.zmax > 0.0 else lvec[3, 2],
    options.dz if options.dz > 0.0 else lvec[3, 2] / nDim[0],
)
print(lvec)
...
np.savetxt(
    fname + "_zlines.dat",
    np.transpose(np.array(dlines)),
    header="# types " + str(atoms[0]),
)
