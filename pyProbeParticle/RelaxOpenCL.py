...
def prepareBuffers(FE, scan_dim):
    ...
    nbytes += FE.nbytes  # TODO make this re-uploadable
    bsz = np.dtype(np.float32).itemsize * 4 * scan_dim[0] * scan_dim[1]
    cl_poss = cl.Buffer(ctx, mf.READ_ONLY, bsz)
    nbytes += bsz  # float4
    cl_FEout = cl.Buffer(ctx, mf.WRITE_ONLY, bsz * scan_dim[2])
    nbytes += bsz * scan_dim[2]  # float4
...
def relax(
    ...
    dpos0=DEFAULT_dpos0,
    relax_params=DEFAULT_relax_params,
):
    ctx, queue = getCtxQueue()
...
class RelaxedScanner:
    ...
    def prepareBuffers(
        ...
        if bFEmap:
            ...
            if FE2in is not None:
                self.cl_ImgFE = cl.image_from_array(
                    self.ctx, FE2in, num_channels=4, mode="r"
                )
                nbytes += FE2in.nbytes
        if verbose > 0:
            print("prepareBuffers.nbytes: ", nbytes)
    ...
    def updateBuffers(self, FEin=None, FE2in=None, lvec=None, WZconv=None):
        ...
        if FE2in is not None:
            region = FE2in.shape[:3]
            region = region[::-1]
            if verbose > 0:
                print("region : ", region)
            cl.enqueue_copy(
                self.queue, self.cl_ImgFE, FE2in, origin=(0, 0, 0), region=region
            )
