...
def setBox(pmin, pmax, k):
    return lib.setBox(
        _np_as(pmin, c_double_p), _np_as(pmax, c_double_p), _np_as(k, c_double_p)
    )
...
def derivGrid_25D(E, F, dx, dy):
    F[:, :, :, 1] = (E[2:, 1:-1, None] - E[:-2, 1:-1, None]) * (0.5 / dx)
    F[:, :, :, 0] = (E[1:-1, 2:, None] - E[1:-1, :-2, None]) * (0.5 / dy)
    F[:, :, :, 2] = 0
    return F
...
def makeGridFF(fff, dx=0.1, dy=0.1):
    ...
    atomMapF = np.empty(
        (atomMap.shape[0] - 2, atomMap.shape[1] - 2, 1, 3), dtype=np.float64
    )
    bondMapF = np.empty(
        (bondMap.shape[0] - 2, bondMap.shape[1] - 2, 1, 3), dtype=np.float64
    )
    atomMapF *= -1
    bondMapF *= -1
    ...
    lvec = np.array(
        [
            [atomMap.shape[0] * dx, 0.0, 0.0],
            [0.0, atomMap.shape[1] * dy, 0.0],
            [0.0, 0.0, 2.0],
        ]
    )
