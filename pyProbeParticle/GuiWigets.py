        ...
        if margins:
            self.axes.add_patch(
                matplotlib.patches.Rectangle(
                    (margins[0], margins[1]),
                    F.shape[1] - margins[2] - margins[0],
                    F.shape[0] - margins[3] - margins[1],
                    linewidth=2,
                    edgecolor="r",
                    facecolor="none",
                )
            ...
            textRes = (
                "output size: "
                + str(F.shape[1] - margins[2] - margins[0])
                + "x"
                + str(F.shape[0] - margins[3] - margins[1])
            )
            if slice_length:
                textRes += "     length [A] =" + "{:03.4f}, {:03.4f}".format(
                    slice_length[0], slice_length[1]
                )
...
class PlotWindow(SlaveWindow):
    ...
    def save_dat(self):
        fileName, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "QFileDialog.getSaveFileName()", "", "data files (*.dat)"
        )
        if fileName:
