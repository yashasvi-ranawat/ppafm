    ...
    for line in fin:
        ...
        if len(words) >= 2:
            ...
            if key in params:
                if key == "stiffness":
                    raise ValueError(
                        "Attention!!! Parameter stifness is deprecated, please define krad and klat instead"
                    )
                val = params[key]
                if key[0][0] == "#":
                    continue
...
def apply_options(opt):
    ...
    for key, value in opt.items():
        ...
        try:
            x = params[
                key
            ]  # to make sure that such a key exists in the list. If not it will be skipped
            params[key] = value
            if verbose > 0:
