...
def readDat(fname, Header=None):
    ...
    Header["ScanPixels_X"] = int(
        SplittedLine.loc[SplittedLine["parameter"] == b"Num.X / Num.X"].value.item()
    )
    Header["ScanPixels_Y"] = int(
        SplittedLine.loc[SplittedLine["parameter"] == b"Num.Y / Num.Y"].value.item()
    )
    Header["GainX"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"GainX / GainX"].value.item()
    )
    Header["GainY"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"GainY / GainY"].value.item()
    )
    Header["GainZ"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"GainZ / GainZ"].value.item()
    )
    Header["GainPreamplifier"] = float(
        SplittedLine.loc[
            SplittedLine["parameter"] == b"Gainpreamp / GainPre 10^"
        ].value.item()
    )
    Header["ChannelCount"] = int(
        SplittedLine.loc[
            SplittedLine["parameter"] == b"Channels / Channels"
        ].value.item()
    )
    Header["DACToZConversionFactor"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"Dacto[A]z"].value.item()
    )
    Header["DACToXYConversionFactor"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"Dacto[A]xy"].value.item()
    )
    Header["ScanRange_X"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"Length x[A]"].value.item()
    )
    Header["ScanRange_Y"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"Length y[A]"].value.item()
    )
    Header["ScanOffset_X"] = float(
        SplittedLine.loc[
            SplittedLine["parameter"] == b"Scanrotoffx / OffsetX"
        ].value.item()
    )
    Header["ScanOffset_Y"] = float(
        SplittedLine.loc[
            SplittedLine["parameter"] == b"Scanrotoffy / OffsetY"
        ].value.item()
    )
    Header["Bias"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"Biasvolt[mV]"].value.item()
    )
    Header["Current"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"Current[A]"].value.item()
    )
    Header["ACQ_Time"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"Sec/Image:"].value.item()
    )
    Header["ScanAngle"] = float(
        SplittedLine.loc[
            SplittedLine["parameter"] == b"Rotation / Rotation"
        ].value.item()
    )
    Header["ZControllerSetpoint"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"FBLogIset"].value.item()
    )
    Header["ZControllerIntegralGain"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"FBIntegral"].value.item()
    )
    Header["ZControllerProportionalGain"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"FBProp"].value.item()
    )
    Header["PiezoX"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"Xpiezoconst"].value.item()
    )
    Header["LengthX"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"Length x[A]"].value.item()
    )
    Header["LengthY"] = float(
        SplittedLine.loc[SplittedLine["parameter"] == b"Length y[A]"].value.item()
    )
    ...
    y_round_size = int(
        np.ceil(float(len(y)) / float(Header["ScanPixels_X"])) * Header["ScanPixels_X"]
    )
