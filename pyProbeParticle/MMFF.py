...
def addBonds(bond2atom_, l0s=None, ks=None):
    return lib.addBonds(
        len(bond2atom_),
        _np_as(bond2atom_, c_int_p),
        _np_as(l0s, c_double_p),
        _np_as(ks, c_double_p),
    )
...
def setBox(pmin, pmax, k):
    return lib.setBox(
        _np_as(np.array(pmin), c_double_p),
        _np_as(np.array(pmax), c_double_p),
        _np_as(np.array(k), c_double_p),
    )
...
def assignAtomTypes(
    ao, nngs, epairProb=0.4, bNitrogenFix=True, ne2elem=["C", "N", "O"]
):
    na = len(ao)
    # --- electron pairs
...
def relaxMolecule(
    ...
    natom = buildFF(
        bAutoHydrogens, bAutoAngles, bSortBonds, bDummyPi, bDummyEpair
    )  # ;print "DEBUG 4"
    ...
    if bAutoHydrogens:
        if bMaskDummy:
            mask_dummy = np.logical_and(
                (types != -2), (types != -3)
            )  # ;print "mask_dummy", mask_dummy
    ...
    if bNonBonded:
        setNonBonded(
            None
        )  # ;print "DEBUG 3"   # this activate NBFF with default atom params
    if bBox:
        setBox(box_pmin, box_pmax, box_k)
