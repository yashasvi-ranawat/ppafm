...
def ringsToMolecule(ring_pos, ring_Rs, Lrange=6.0):
    ...
    ring_nngs = np.array(
        [len(ng) for ng in ring_neighs], dtype=np.int
    ...
    atom_pos = (
        ring_pos[atom2ring[:, 0]]
        + ring_pos[atom2ring[:, 1]]
        + ring_pos[atom2ring[:, 2]]
    ) / 3.0
    ...
    ring_natm = getRingNatom(
        atom2ring, len(ring_neighs)
    )  # ;print "ring_natm ", ring_natm
    ring_N6mask = np.logical_and(
        ring_natm[:] == 6, ring_nngs[:] == 6
    )  # ;print "ring_N6mask", len(ring_N6mask), ring_N6mask
    atom_N6mask = np.logical_or(
        ring_N6mask[atom2ring[:, 0]],
        np.logical_or(ring_N6mask[atom2ring[:, 1]], ring_N6mask[atom2ring[:, 2]]),
        ...
        if name in group_definition:
            ...
            if ndir == 4:  # ==== tetrahedral
                ...
                if nsigma == 1:  # like -CH3
                    ...
                    txyz = (
                        makeTetrahedronFork(pi - ps[ngs[0]], pi - ps[ngs[1]])
                        + pi[None, :]
                    )
                ...
                if nsigma == 1:  # like =CH2
                    ...
                    appendHs(
                        normalize(pi * 2 - ps[ngs[0]] - ps[ngs[1]])[0] + pi[None, :],
                        [(1,)],
                        elems,
                        xyzs,
                    )
                ...
                appendHs(
                    normalize(pi - ps[ngs[0]])[0] + pi[None, :], [(1,)], elems, xyzs
                )
                # if   (nsigma==1):
                #    elems.append("H")
...
class FIRE:
    ...
    def __init__(
        self, dt_max=0.2, dt_min=0.01, damp_max=0.2, f_limit=10.0, v_limit=10.0
    ):
        self.dt = dt_max
        self.dt_max = dt_max
        self.dt_min = dt_min
    ...
    def move(self, p, f):
        ...
        vf = np.dot(v, f)
        dt_sc = min(
            min(1.0, self.f_limit / (f_norm + 1e-32)),
            min(1.0, self.v_limit / (v_norm + 1e-32)),
        )
...
def estimateBondOrder(atypes, bonds, E12=0.5, E22=+0.5, E32=+0.5):
    ...
    bo, ao = relaxBondOrder(
        bonds, typeMasks, typeFFs, fConv=0.0001, optimizer=opt, EboStart=0.0, EboEnd=0.0
    )  # relax delocalized pi-bond superposition
    bo, ao = relaxBondOrder(
        bonds,
        typeMasks,
        typeFFs,
...
def relaxAtoms(
    ps, aParams, FFfunc=getForceIvnR24, fConv=0.001, nMaxStep=1000, optimizer=None
):
    if optimizer is None:
        optimizer = FIRE()
