...
def initArgsLJC(atoms, cLJs, poss):
    ...
    cl_FE = cl.Buffer(ctx, mf.WRITE_ONLY, poss.nbytes)
    nbytes += poss.nbytes  # float4     # we are using Qmix now
...
def runCoulomb(kargs, nDim, local_size=(32,)):
    ...
    ntot = makeDivisibleUp(
        ntot, local_size[0]
    )  # TODO: - we should make sure it does not overflow
    global_size = (ntot,)
...
def runLJ(
    kargs, nDim, local_size=(32,)
):  # slowed down, because of problems with the field far away
    ctx, queue = getCtxQueue()
    ntot = nDim[0] * nDim[1] * nDim[2]
    ntot = makeDivisibleUp(
        ntot, local_size[0]
...
def getPos(lvec, nDim=None, step=(0.1, 0.1, 0.1)):
    if nDim is None:
        nDim = (
            int(np.linalg.norm(lvec[3, :]) / step[2]),
...
class ForceField_LJC:
    ...
    def setLvec(self, lvec, nDim=None):
        if nDim is not None:
            ...
            print("ERROR : nDim must be set somewhere")
            exit()
    ...
    def prepareBuffers(self, atoms=None, cLJs=None, poss=None, bDirect=False, nz=20):
        ...
        if atoms is not None:
            ...
            self.cl_atoms = cl.Buffer(
                self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=atoms
            )
            nbytes += atoms.nbytes
        if cLJs is not None:
            self.cl_cLJs = cl.Buffer(
                self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=cLJs
            )
            nbytes += cLJs.nbytes
        if poss is not None:
            self.nDim = np.array(poss.shape, dtype=np.int32)
            self.cl_poss = cl.Buffer(
                self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=poss
            )
            nbytes += poss.nbytes  # float4
    ...
    def run(self, FE=None, local_size=(32,), bCopy=True, bFinish=True):
        ...
        if bRuntime:
            print("runtime(ForceField_LJC.evalLJC.pre) [s]: ", time.clock() - t0)
        ...
        if bFinish:
            self.queue.finish()
        if bRuntime:
            print("runtime(ForceField_LJC.evalLJC) [s]: ", time.clock() - t0)
        return FE

    def run_evalLJC_Q(
        self, FE=None, Qmix=0.0, local_size=(32,), bCopy=True, bFinish=True
    ):
        """
        generate force-field
        """
        ...
        if bRuntime:
            print("runtime(ForceField_LJC.run_evalLJC_Q.pre) [s]: ", time.clock() - t0)
        ...
        if bFinish:
            self.queue.finish()
        if bRuntime:
            print("runtime(ForceField_LJC.run_evalLJC_Q) [s]: ", time.clock() - t0)
        return FE

    def run_evalLJC_QZs_noPos(
        self, FE=None, Qmix=0.0, local_size=(32,), bCopy=True, bFinish=True
    ):
        """
        generate force-field
        ...
        if bRuntime:
            print(
                "runtime(ForceField_LJC.run_evalLJC_QZs_noPos.pre) [s]: ",
                time.clock() - t0,
            )
        ...
        if bRuntime:
            print(
                "runtime(ForceField_LJC.run_evalLJC_QZs_noPos) [s]: ", time.clock() - t0
            )
        return FE
    ...
    def run_evalLJC_Q_noPos(
        ...
        if bRuntime:
            print(
                "runtime(ForceField_LJC.run_evalLJC_Q_noPos.pre) [s]: ",
                time.clock() - t0,
            )
        ...
        if bFinish:
            self.queue.finish()
        if bRuntime:
            print("runtime(ForceField_LJC.evalLJC_Q_noPos) [s]: ", time.clock() - t0)
        return FE
    ...
    def makeFF(
        ...
        if bRuntime:
            print("runtime(ForceField_LJC.makeFF.pre) [s]: ", time.clock() - t0)
        # FF = self.run( FE=FE, Qmix=Qmix, bCopy=bCopy, bFinish=bFinish )
        if self.cl_poss is not None:
            FF = self.run_evalLJC_Q(
                FE=FE, Qmix=Qmix, local_size=(32,), bCopy=bCopy, bFinish=bFinish
            )
        else:
            if bQZ:
                FF = self.run_evalLJC_QZs_noPos(
                    FE=FE, Qmix=Qmix, local_size=(32,), bCopy=bCopy, bFinish=bFinish
                )
            else:
                FF = self.run_evalLJC_Q_noPos(
                    FE=FE, Qmix=Qmix, local_size=(32,), bCopy=bCopy, bFinish=bFinish
                )
        ...
        if bRuntime:
            print("runtime(ForceField_LJC.makeFF.tot) [s]: ", time.clock() - t0)
        return FF, atoms
...
class AtomProcjetion:
    ...
    def prepareBuffers(self, atoms, prj_dim, coefs=None, bonds2atoms=None, Rfunc=None):
        ...
        if (Rfunc is not None) or (self.Rfunc is not None):
            ...
            self.cl_Rfunc = cl.Buffer(
                self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=Rfunc
            )
            # exit(0);
        ...
        if bonds2atoms is not None:
            ...
            self.cl_bondPoints = cl.Buffer(
                self.ctx, mf.READ_ONLY | mf.COPY_HOST_PTR, hostbuf=bondPoints
            )
        ...
        self.cl_itypes = cl.Buffer(
            self.ctx, mf.READ_ONLY, 200 * np.dtype(np.int32).itemsize
        )
        nbytes += bsz  # float
    ...
    def run_evaldisks(
        self, poss=None, Eout=None, tipRot=None, offset=0.0, local_size=(32,)
    ):
        """
    ...
    def run_evaldisks_occlusion(
        self, poss=None, Eout=None, tipRot=None, local_size=(32,)
    ):
        """
    ...
    def run_evalSpheresType(
        self, poss=None, Eout=None, tipRot=None, bOccl=0, local_size=(32,)
    ):
        """
