...
def loadAtoms(name):
    ...
    try:
        ...
        raise ValueError(
            "First line of a xyz file should contain the number of atoms. Aborting..."
        )
    line = f.readline()
    if n > 0:
        n = int(l)
...
def loadGeometry(fname=None, params=None):
    ...
    if is_xyz:
        ...
        sys.exit(
            "ERROR!!! Unknown format of geometry system. Supported formats are: .xyz, .cube, .xsf \n\n"
        )
