    ...
    for iatom, apos in enumerate(atom_pos):
        if atom_mask[iatom]:
            ...
            if radial_func is not None:
                radial = radial_func(
                    R, beta
                )  # TODO: problem is that radial function is different for  monopole, dipole, quadrupole ...
...
cpp_utils.make("MP")
lib = ctypes.CDLL(
    cpp_utils.CPP_PATH + "/" + cpp_name + cpp_utils.lib_ext
)  # load dynamic librady object using ctypes
...
lib.sampleGridArroundAtoms.argtypes = [
    c_int,
    array2d,
    array1d,
    array1d,
...
def sampleGridArroundAtoms(
    atom_pos, atom_Rmin, atom_Rmax, atom_mask, pbc=False, show_where=False
):
    natom = len(atom_pos)
    ...
    points_found = lib.sampleGridArroundAtoms(
        natom,
        atom_pos,
        atom_Rmin,
        atom_Rmax,
...
lib.buildLinearSystemMultipole.argtypes = [
    c_int,
    array2d,
    array1d,
    array1d,
...
lib.regularizedLinearFit.argtypes = [
    c_int,
    array1d,
    array1d,
    array1d,
...
def regularizedLinearFit(
    B, BB, coefs=None, kReg=1.0, dt=0.1, damp=0.1, convF=1e-6, nMaxSteps=1000
):
    n = len(B)
...
def fitMultipolesPotential(
    ...
    sampled_val, sampled_pos = sampleGridArroundAtoms(
        atom_pos, atom_Rmin, atom_Rmax, atom_mask, pbc=pbc, show_where=show_where
    )
    # print "bas_list:", atom_bas
    X = sampled_pos[:, 0]
    Y = sampled_pos[:, 1]
    Z = sampled_pos[:, 2]
    basis_set, basis_assignment = sample_basis(
        atom_pos, atom_basis, atom_mask, X, Y, Z, radial_func=None, beta=1.0
    )
