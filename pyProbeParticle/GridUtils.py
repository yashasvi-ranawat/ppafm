...
lib.interpolateQuad_gridCoord.argtypes = [
    array1i,
    array1d,
    array1d,
    array1d,
...
def loadCUBE(fname):
    ...
    print(nDim)
    FF = (
        np.reshape(F, nDim).transpose((2, 1, 0)).copy()
    )  # Transposition of the array to have the same order of data as in XSF file
...
def saveVecFieldNpy(fname, FF, lvec, head=XSF_HEAD_DEFAULT):
    ...
    if head != XSF_HEAD_DEFAULT:
        ...
        # head: [e,[x,y,z],lvec]
        np.save(
            fname + "_atoms.npy", [tmp0, head[1][0], head[1][1], head[1][2], q]
        )  # atoms: [e, x, y, z, q]
