...
class OCLEnvironment:
    def __init__(self, i_platform=0):
        ...
        print(
            "OCLEnvironment platform[%i]" % i_platform,
            " PACKAGE_PATH: ",
            self.PACKAGE_PATH,
        )
        ...
        self.ctx = cl.Context(
            properties=[(cl.context_properties.PLATFORM, self.platform)], devices=None
        )
    ...
    def loadProgram(self, fname):
        ...
        cl._DEFAULT_INCLUDE_OPTIONS.append(
            "-I" + self.CL_PATH
        )  # this is a bit a hack !!!   not sure how to add include dir properly https://documen.tician.de/pyopencl/runtime_program.html#program
    ...
    def updateBuffer(self, buff, cl_buff, access=cl.mem_flags):
        if buff is not None:
            if cl_buff is None:
                cl_buff = cl.Buffer(
                    self.ctx, access | cl.mem_flags.COPY_HOST_PTR, hostbuf=buff
                )
                return buff.nbytes
            else:
    ...
    def printPlatformInfo(self):
        ...
        for device in platform.get_devices():
            ...
            print(
                "  GLOBAL_MEM_SIZE          = ",
                device.get_info(cl.device_info.GLOBAL_MEM_SIZE) / 4,
                " float32",
            )
            print(
                "  LOCAL_MEM_SIZE           = ",
                device.get_info(cl.device_info.LOCAL_MEM_SIZE) / 4,
                " float32",
            )
            print(
                "  MAX_CONSTANT_BUFFER_SIZE = ",
                device.get_info(cl.device_info.MAX_CONSTANT_BUFFER_SIZE) / 4,
                " float32",
            )
            print(
                "  MAX_WORK_GROUP_SIZE      = ",
