...
class Generator(
    ...
    def __init__(
        ...
        if (
            self.Ymode == "Lorenzian"
            or self.Ymode == "Spheres"
            or self.Ymode == "SphereCaps"
            or self.Ymode == "Disks"
            or self.Ymode == "DisksOcclusion"
            or self.Ymode == "QDisks"
            or self.Ymode == "D-S-H"
            or self.Ymode == "MultiMapSpheres"
        ):
        ...
        self.scanner.relax_params = np.array(
            [0.1, 0.9, 0.1 * 0.2, 0.1 * 5.0], dtype=np.float32
        )
        self.scanner.stiffness = (
            np.array([0.24, 0.24, 0.0, 30.0], dtype=np.float32) / -16.0217662
        )
    ...
    def genMolRffRandom(self):
        ...
        rff.clean()
        self.rff_itypes = (np.random.rand(self.rff_natom) * 1.3).astype(np.int32)
        # print "itypes", self.rff_itypes
    ...
    def nextMolecule(self, fname):
        ...
        if self.use_rff:
            ...
            self.relaxMolRff()
            xyzs, itypes_ = rff.h2bonds(
                self.rff_itypes, self.rff_poss, self.rff_hbonds, bsc=1.1
            )
            xyzs, itypes_ = rff.removeSaturatedBonds(self.rff_caps, itypes_, xyzs)
            self.atom_lines = []
        ...
        setBBoxCenter(
            xyzs, (self.lvec[1, 0] * 0.5, self.lvec[2, 1] * 0.5, self.lvec[3, 2] * 0.5)
        )
        self.natoms0 = len(Zs)
        ...
        if self.npbc is not None:
            Zs, xyzs, qs, cLJs = PPU.PBCAtoms3D(
        ...
        FF, self.atoms = self.forcefield.makeFF(
            xyzs, qs, cLJs, lvec=self.lvec, pixPerAngstrome=self.pixPerAngstrome
        )
        ...
        if self.projector is not None:
            ...
            if self.Ymode == "MultiMapSpheres":
                self.projector.prepareBuffers(
                    self.atomsNonPBC, self.scan_dim[:2] + (8,), coefs=coefs
                )
            else:
                self.projector.prepareBuffers(
    ...
    def nextRotation(self, X, Y):
        ...
        self.scan_pos0s = self.scanner.setScanRot(
            self.pos0 + self.rot[2] * self.distAbove,
            rot=self.rot,
        ...
        if self.Ymode == "HeightMap":
            ...
            if verbose > 0:
                print("dirFw ", dirFw)
            poss_ = np.float32(
                self.scan_pos0s - (dirFw * (self.distAbove - 1.0))[None, None, :]
            )
        ...
        if self.debugPlots:
            ...
            self.plot(
                ("/rot%03i_" % self.irot), self.molName, X=X, Y=Y, entropy=entropy
            )
    ...
    def getAFMinRot(self, rot, X):
        ...
        self.scan_pos0s = self.scanner.setScanRot(
            self.pos0 + rot[2] * self.distAbove,
            rot=rot,
            start=self.scan_start,
            end=self.scan_end,
            tipR0=tipR0,
    ...
    def plot(
        ...
        if bXYZ:
            ...
            if bRot:
                ...
                basUtils.writeDebugXYZ__(
                    self.preName + molName + rotName + ".xyz", atomsRot, self.Zs
                )
                # exit()
            else:
                basUtils.writeDebugXYZ_2(
                    self.preName + molName + rotName + ".xyz",
                    self.atoms,
                    self.Zs,
        ...
        if Y is not None:
            ...
            if self.Ymode == "D-S-H":
                ...
                plt.subplot(1, 3, 1)
                plt.imshow(Y[:, :, 0], origin="image")
                plt.title("Disks")
                plt.colorbar()
                plt.subplot(1, 3, 2)
                plt.imshow(Y[:, :, 1], origin="image")
                plt.title("Spheres")
                plt.colorbar()
                plt.subplot(1, 3, 3)
                plt.imshow(Y[:, :, 2], origin="image")
                plt.title("HeightMap")
                plt.colorbar()
            else:
        ...
        for isl in self.debugPlotSlices:
            ...
            if (X is not None) and (Y_ is not None):
                ...
                plt.subplot(1, 2, 2)
                plt.imshow(X[:, :, isl], origin="image")
                plt.colorbar()
                # plt.subplot(1,2,1); plt.imshow (Y_[:,:,isl], origin='image' );
                plt.subplot(1, 2, 1)
                plt.imshow(np.tanh(Y_[:, :, isl]), origin="image")
                plt.colorbar()
                plt.title(title)
                plt.savefig(
                ...
                if X is not None:
                    ...
                    plt.close()
                if Y_ is not None:
                    plt.imshow(Y_[:, :, isl], origin="image")
                    plt.colorbar()
