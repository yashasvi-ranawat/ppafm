...
def getProbeDensity(sampleSize, X, Y, Z, dd, sigma=0.7, multipole_dict=None, tilt=0.0):
    ...
    radial_renom = (
        np.sum(radial) * np.abs(np.linalg.det(mat)) * dd[0] * dd[1] * dd[2]
    )  # TODO analytical renormalization may save some time ?
    radial /= radial_renom
    if (
        multipole_dict is not None
    ):  # multipole_dict should be dictionary like { 's': 1.0, 'pz':0.1545  , 'dz2':-0.24548  }
        ...
        for kind, coef in multipole_dict.items():
            ...
            rho += (
                ...
                * getSphericalHarmonic(
                    rx / sigma, ry / sigma, rz / sigma, kind=kind, tilt=tilt
                )
            )
    else:
        rho = radial
...
def addCoreDensities(atoms, valElDict, rho, lvec, sigma=0.1):
    ...
    for ia in range(len(atoms[0])):
        ...
        xyz = atoms[1:4, ia]
        addCoreDensity(
            rho, -nel / dV, rx - xyz[0], ry - xyz[1], rz - xyz[2], sigma=sigma
        )
...
def printMetadata(sampleSize, dims, dd, xsize, ysize, zsize, V, rho):
    ...
    print(
        "number of data points:".rjust(first_col),
        " dims".rjust(sec_col),
        " = %s" % list(dims),
    )
    print(
        "specimen size:".rjust(first_col),
        "(xsize, ysize, zsize)".rjust(sec_col),
        " = (%s, %s, %s)" % (xsize, ysize, zsize),
    )
    print(
        "elementary lengths:".rjust(first_col),
        "(dx, dy, dz)".rjust(sec_col),
        " = (%.5f, %.5f, %.5f)" % dd,
    )
    print(
        "V potential:".rjust(first_col),
        "(max, min)".rjust(sec_col),
        " = (%s, %s)" % (V.max(), V.min()),
    )
    print("".rjust(first_col), "V.shape".rjust(sec_col), " = %s" % list(V.shape))
    print(
        "probe potential:".rjust(first_col),
        "(max, min)".rjust(sec_col),
        " = (%s, %s)" % (rho.max(), rho.min()),
    )
...
def potential2forces(V, lvec, nDim, sigma=0.7, rho=None, multipole=None, tilt=0.0):
    ...
    if rho == None:
        ...
        rho = getProbeDensity(
            sampleSize, X, Y, Z, dd, sigma=sigma, multipole_dict=multipole, tilt=tilt
        )
    else:
        rho[:, :, :] = rho[::-1, ::-1, ::-1].copy()
...
def potential2forces_mem(
    ...
    if doForce:
        ...
        zetaX, zetaY, zetaZ, detLmatInv = getForceTransform(
            sampleSize, dims, dd, X, Y, Z
        )
    del X, Y, Z
    E = None
    Fx = None
