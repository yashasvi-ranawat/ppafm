...
def getAtomsRotZmin(rot, xyzs, zmin, Zs=None):
    ...
    xyzs_[:, 0] = (
        rot[0, 0] * xyzs[:, 0] + rot[0, 1] * xyzs[:, 1] + rot[0, 2] * xyzs[:, 2]
    )
    xyzs_[:, 1] = (
        rot[1, 0] * xyzs[:, 0] + rot[1, 1] * xyzs[:, 1] + rot[1, 2] * xyzs[:, 2]
    )
    xyzs_[:, 2] = (
        rot[2, 0] * xyzs[:, 0] + rot[2, 1] * xyzs[:, 1] + rot[2, 2] * xyzs[:, 2]
    )
    mask = xyzs_[:, 2] > zmin
...
class Generator(
        ...
        for ibatch in range(n):
            ...
            if self.irot == 0:  # recalc FF
                ...
                if self.bDfPerMol:
                    ...
                    print(" ============= ndf ", ndf)
                    self.dfWeight = PPU.getDfWeight(ndf, dz=self.scanner.zstep).astype(
                        np.float32
                    )
    ...
    def nextRotation(self, X, Y):
        ...
        self.forcefield.cl_points = (
            self.scanner.cl_poss
        )  # starting positions -> forcefield
        self.forcefield.cl_FE = self.scanner.cl_FEout
        self.forcefield.scan_dim = self.scanner.scan_dim
        FE = self.forcefield.runRelaxStrokesDirect(
            self.Q,
            self.scanner.cl_FEout,
            FE=None,
            local_size=(32,),
            nz=self.scanner.scan_dim[2],
        )
