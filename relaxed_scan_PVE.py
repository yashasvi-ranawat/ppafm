...
if __name__ == "__main__":
    ...
    parser = OptionParser()
    parser.add_option(
        "-k", "--klat", action="store", type="float", help="tip stiffenss [N/m]"
    )
    parser.add_option(
        "--krange",
        action="store",
    ...
    parser.add_option(
        "-q", "--charge", action="store", type="float", help="tip charge  [e]"
    )
    parser.add_option(
        "--Apauli",
        default=PPU.params["Apauli"],
        action="store",
        type="float",
        help="FFpauli scaling; i.e. prefactor A in formula E = A*Integral( rho_tip^B * rho_sample^B ) ",
    )
    parser.add_option(
        "--qrange",
        action="store",
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="store total Force-Field for debuging",
    )
    parser.add_option(
        "--pos",
        action="store_true",
        default=False,
        help="save probe particle positions",
    )
    parser.add_option(
        "--disp",
        action="store_true",
        default=False,
        help="save probe particle displacements",
    )
    parser.add_option(
        "--vib",
        action="store",
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        help="file where spline is stored",
        default=None,
    )
    parser.add_option(
        "--rotate",
    ...
    parser.add_option(
        ...
        action="store",
        type="string",
        help="Specify the input/output format of the vector and scalar field. Supported formats are: xsf,npy",
        default="xsf",
    )
    ...
    if options.boltzmann or options.bI:
        ...
        FFboltz, lvec, nDim = GU.load_vec_field(
            "FFboltz", data_format=options.data_format
        )
    ...
    for iq, Q in enumerate(Qs):
        ...
        for ik, K in enumerate(Ks):
            ...
            fzs, PPpos, PPdisp, lvecScan = PPH.perform_relaxation(
                ...
                FFpauli=FFpauli,
                FFboltz=FFboltz,
                tipspline=options.tipspline,
                bFFtotDebug=options.bDebugFFtot,
            )
            ...
            GU.save_scal_field(
                dirname + "/OutFz", fzs, lvecScan, data_format=options.data_format
            )
            if opt_dict["vib"] >= 0:
                which = opt_dict["vib"]
                print(
            ...
            if opt_dict["pos"]:
                GU.save_vec_field(
                    dirname + "/PPpos", PPpos, lvecScan, data_format=options.data_format
                )
            if options.bI:
                print("Calculating current from tip to the Boltzmann particle:")
                I_in, lvec, nDim = GU.load_scal_field(
                    "I_boltzmann", data_format=iptions.data_format
                )
