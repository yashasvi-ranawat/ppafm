...
Q = -0.1
bPBC = True
lvec = np.array([[0.0, 0.0, 0.0], [19.0, 0.0, 0.0], [0.0, 20.0, 0.0], [0.0, 0.0, 21.0]])
...
if __name__ == "__main__":
    rotations = hl.PPU.genRotations(
        np.array([1.0, 0.0, 0.0]), np.linspace(-np.pi / 2, np.pi / 2, 8)
    )
    # print "rotations: ", rotations

    typeParams = hl.loadSpecies("atomtypes.ini")
    invCell = hl.oclr.getInvCell(lvec)
    ff_dim = hl.genFFSampling(lvec, pixPerAngstrome)
    print("ff_dim ", ff_dim)
    poss = hl.FFcl.getposs(lvec, ff_dim)
    ...
    for dirName in dirNames:
        ...
        FF, atoms, natoms0 = hl.makeFF_LJC(
            poss, atom_lines, typeParams, iZPP, lvec, npbc=(1, 1, 1)
        )
        FEin = FF[:, :, :, :4] + Q * FF[:, :, :, 4:]
        del FF
        # Tff = time.clock()-t1ff;
        ...
        for irot, rot in enumerate(rotations):
            ...
            FEout = hl.scanFromRotation(
                ...
                debugXYZ=(subDirName + "/debugPos0.xyz", atom_lines),
            )
