...
parser.add_option(
    ...
    action="store",
    type="string",
    default="./tip/CHGCAR.xsf",
    help="tip 3D data-file (.xsf)",
)
parser.add_option(
    "-o",
    "--output",
...
parser.add_option(
    ...
    type="float",
    default="-1.0",
    help="exponent B in formula E = A*Integral( rho_tip^B * rho_sample^B ); NOTE: negative value equivalent to B=1 ",
)
parser.add_option(
    "-A",
    "--Apauli",
    ...
    type="float",
    default="1.0",
    help="prefactor A in formula E = A*Integral( rho_tip^B * rho_sample^B ); NOTE: default A=1 since re-scaling done in relax_scan_PVE.py",
)
parser.add_option(
    "-E",
    "--energy",
    action="store_true",
    help="Compue potential energ y(not just Force)",
    default=False,
)
parser.add_option(
    "--saveDebugXsfs",
    action="store_true",
    help="save auxuliary xsf files for debugging",
    default=False,
)
parser.add_option(
    "--densityMayBeNegative",
    action="store_false",
    help="input desnity files from DFT may contain negative voxels, lets handle them properly",
    default=True,
)
...
if np.any(nDimS != nDimT):
    raise Exception(
        "Tip and Sample grids has different dimensions! - sample: "
        + str(nDimS)
        + " tip: "
        + str(nDimT)
    )
if np.any(lvecS != lvecT):
    raise Exception(
        "Tip and Sample grids has different shap! - sample: "
        + str(lvecS)
        + " tip: "
        + str(lvecT)
    )
...
print(
    ">>> Evaluating convolution E(R) = A*Integral_r ( rho_tip^B(r-R) * rho_sample^B(r) ) using FFT ... "
)
Fx, Fy, Fz, E = fFFT.potential2forces_mem(
    rhoS, lvecS, nDimS, rho=rhoT, doForce=True, doPot=True, deleteV=True
)
